...
```{r fig.align='center', echo=FALSE, fig.cap = "Law of Diminishing Marginal Utility", label='fig-marginalutility', out.width="600px"}
par(mar = c(4, 4, 0.1, 0.1))
x <- seq(1, 20, 1)
y <- x^(0.2)
plot(x, y, pch = 19, type = "b", xlab="Number of units consumed from the same good", ylab="Utility", yaxt='n')
```
...
```{r fig.align='center', echo=FALSE, fig.cap = "Karl Popper", label='fig-popper', out.width="300px"}
...
```{r fig.align='center', echo=FALSE, fig.cap = "Version confusion", label='fig-version', out.width="600px"}
...
As quantitative research becomes more and more important in the social sciences you need software to **manage** your data and provides statistical and applied econometric **analyses**. We will use `R` in this course. However, note that `R`  has a steeper learning curve than e.g., `STATA`, and does not work immediately out-of-the-box. But the user base is large and that is important, because for each problem there is much material to be found on internet (including videos). In this syllabus we will give specific attention to `R`-script, including why you code it like such and what the intuition is behind that.

### Where to get `R`

First, you need to install `R` itself. You can do this by downloading this from [CRAN](https://cran.r-project.org/mirrors.html) (choose the server you want). Now choose your appropriate operating system, choose the `base` system, download `R` and install it. That's it!

The base distribution of `R` comes with a built-in editor, where you can write your script. This editor is however *very* basic. Therefore, it is very much advised that you download and install the free editor [`RStudio`](https://posit.co/download/rstudio-desktop/) as well. Again, choose your operating system and just install the latest version. The very short video (6 minutes) [website](https://www.youtube.com/watch?v=FIrsOBy5k58) gives an overview of the *basic* features (it can do quite some more stuff). 

### Why use `R` and not any other application?
...
[[CRAN packages](https://cran.r-project.org/web/packages/available_packages_by_name.html) give a great overview of all the official packages out there and the wide range of applications, and again they are all free!]{.aside}
